...
#1 Cargamos los conjuntos de datos que usaremos

data(package = "MASS", topic = "mammals")
data(package = "robustbase", topic = "Animals2")

#2 Miramos las caracteristicas de cada conjunto de datos con help

help(package = "MASS", topic = "mammals")
help(package = "robustbase", topic = "Animals2")

#3 Aplicamos las funciones  dim(), head(), tail() y str()
...
#4 Mostramos el nombre de las filas y las columnas de cada conjutno de datos

#MASS::mammals

mammals_row <- rownames(MASS::mammals)
mammals_row

mammals_col <- colnames(MASS::mammals)
mammals_col

#robustbase::Animals2

animals2_row <- rownames(robustbase::Animals2)
animals2_row

animals2_col <- colnames(robustbase::Animals2)
animals2_col
...
commonAnimals <- intersect(mammals_row, animals2_row)
#Los animales que aparecen en ambos conjuntos son
commonAnimals
...
AnimalesNoEstan1 <- setdiff(mammals_row, animals2_row)
AnimalesNoEstan1

AnimalesNoEstan2 <- setdiff(animals2_row, mammals_row)
AnimalesNoEstan2

#Podemos observar que hay 4 animales que no estan en ninguno de los dos conjuntos en el primer caso, mientras que en el segundo caso hay 8 animales que no estan, siendo animales muy variados"
...
#Podemos observar que los animales que no aparecen en ambos conjuntos es debido a que se han escrito de manera diferente en uno y en otro como en el caso de: "Artic fox" y "Arctic fox"
...
car::qqPlot(brain_weights_mammals)
car::qqPlot(brain_weights_animals2)

#Podemos observar que presentan el mismo comportamientos ya que ambas estan dentro del rango de las lineas discontinuas y porque tanto siguen una distribucion gaussiana
...
#Como en ambos casos los valores (Puntos) se encuentran dentro del rango de las lineas discontinuas podemos verificar que cumplen una distribucion gaussiana
...
```{r}
Variedad <- data.frame(cabbages$Cult)
Variedad
VitaminaC <- cabbages$VitC
hist(VitaminaC, col = "lightblue")
#hist(Variedad, col = "lightblue")
...
a.  ggplot(cabbages, aes(VitC,HeadWt)) +

    geom_point() + geom_smooth()

b.  Utiliza un procedimiento análogo para determinar si se aprecia una relación lineal entre los niveles de vitamina C, **VitC** en función del peso de la calabaza, **HeadWt**, en el conjunto de datos **cabbages**.

    ```{r}
    ggplot(cabbages, aes(VitC,HeadWt)) +
      geom_point() + geom_smooth()

    ```

c.  Repite el apartado anterior, pero obteniendo un modelo para cada una de las dos variedades de calabaza, **Cult**. Ver[Parámetros básicos plot](https://www.statmethods.net/advgraphs/parameters.html).

```{r}

ggplot(cabbages, aes(Cult[1],VitC,HeadWt)) +
  geom_point() 

ggplot(cabbages, aes(Cult[2],VitC,HeadWt)) +
  geom_point() 
...
```{r}
summary(ggplot(cabbages, aes(VitC,HeadWt)) +
  geom_point() + geom_smooth()) 

summary(ggplot(cabbages, aes(Cult[1],VitC,HeadWt)) +
  geom_point() )


summary(ggplot(cabbages, aes(Cult[2],VitC,HeadWt)) +
  geom_point() )
```
